...
def write_model(model_path, input_base_path, model_size):
    ...
    for layer_i in range(n_layers):
        ...
        #torch.save(state_dict, os.path.join(tmp_model_path, filename), torch_dtype=torch.bfloat16)
    ...
    #model = LlamaForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.float8, low_cpu_mem_usage=True)
    # Avoid saving this as part of the config.
    del model.config._name_or_path

    print("Saving in the Transformers format.")
    #model.save_pretrained(model_path)
    model.save_pretrained(model_path, torch_dtype=torch.bfloat16)
